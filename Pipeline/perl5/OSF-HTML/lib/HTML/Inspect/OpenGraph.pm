...
my %default_prefixes = (og => 'https://ogp.me/ns#', map +($_ => "https://ogp.me/ns/$_#"), @sub_types,);
...
sub collectOpenGraph ($self, %args) {
    ...
    foreach my $meta ($find_meta_property->($self)) {
        ...
        my $prefix = $prefer{$used_prefix} || (exists $default_prefixes{$used_prefix} ? $used_prefix : next);
